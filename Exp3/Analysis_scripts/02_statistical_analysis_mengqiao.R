...
######### task transform paradigm : rule transform ########## 
library(here)
library(tidyverse)
library(ez)
library(feather)
...
###########################################################
# load(file = "D:/Ghent_Braem/Experiment/2nd_experiment/Experiment_rule_tran/result/bigdf_clean.Rdata")
...
## plotting for publication ----
# for the task rule congruency effect
...
# for the stimulus type congruency effect
...
model_rt_max <- lmer(formula = rt ~ (CTI_sec + incongruence_score_stim + incongruence_score_rule) * block_type_binary
                     ...
                     data = regular_trials_lme,
                     REML = TRUE,
                     control = lmerControl(optimizer = "nloptwrap", # bobyqa and Nelder_Mead not working
                                           calc.derivs = FALSE,
                                           optCtrl = list(maxfun = 2e5)))

anova(model_rt_max)
summary(model_rt_max, corre)
...
###########################################################
# load(file = "D:/Ghent_Braem/Experiment/2nd_experiment/Experiment_rule_tran/result/bigdf_clean_error.Rdata")
...
tableALL_ready
save_as_docx(tableALL_ready, path = paste0(here(), "/APA_table_rule_tran_sm.docx"))
...
cor_model_rt <- cor.test(tran_sub$`CTI_sec:block_type_binary`, tran_sub$meanRT, method = "pearson")
cor_model_rt

cor_model_acc <- cor.test(tran_sub$`CTI_sec:block_type_binary`, tran_sub$accRate, method = "pearson")
cor_model_acc
