...
postsamples_files <- list.files(paste0(here(), "/allExp_allCon/full_results"), 
...
read_csv_filename <- function(filename){
  onedf <- read_csv(filename) %>%
    mutate(file_name = filename,
           ...
           effect = case_when(
             str_detect(filename, "CTI") ~ "CTI",
             str_detect(filename, "ruleCon") ~ "ruleCon",
             str_detect(filename, "stimCon") ~ "stimCon"          
           )) %>%
    select(! contains("subj"))
  return(onedf)
}
...
save(big_ddm_postsamples, file = paste0(here(), "/results/big_ddm_postsamples.RData"))
...
load(paste0(here(), "/results/big_ddm_postsamples.RData"))

##- for CTI effects of DDM parameters between v2 and control ----
psamples <- big_ddm_postsamples %>%
  filter(exp == "v2" | exp == "control") %>%
  filter(effect == "CTI") %>%
  select(exp,
         "a_CTI:C(block_type, Treatment('RG'))[T.TF]",
         "v_CTI:C(block_type, Treatment('RG'))[T.TF]",
...
dist_post1 <- big_ddm_postsamples %>% # statistics of the posterior samples of experiment v2
  filter(exp == "v2") %>%
  filter(effect == "CTI") %>%
  select("a_CTI:C(block_type, Treatment('RG'))[T.TF]",
         "v_CTI:C(block_type, Treatment('RG'))[T.TF]",
         "t_CTI:C(block_type, Treatment('RG'))[T.TF]") %>%
...
dist_post2 <- big_ddm_postsamples %>% # statistics of the posterior samples of control experiment
  filter(exp == "control") %>%
  filter(effect == "CTI") %>%
  select("a_CTI:C(block_type, Treatment('RG'))[T.TF]",
         "v_CTI:C(block_type, Treatment('RG'))[T.TF]",
...
( # decision boundary
p1<- psamples %>%
  filter(parameters == "a_CTI:C(block_type, Treatment('RG'))[T.TF]") %>%
  ...
  scale_x_discrete(labels = c("Control Exp.", "Exp. 1")) +
  ggtitle("Threshold(a)") +
  theme_apa(base_size = 14) +
...
# compute the difference of decision boundary between control and v2
...
a_diffDist <- psamples %>%
  ...
  select(-row) %>%
  mutate(diff = v2 - control)

a <- hdi(a_diffDist$diff, ci = 0.95, verbose = TRUE)
b <- hdi(a_diffDist$diff, ci = 0.89, verbose = TRUE)


( # drift rate
p2<- psamples %>%
  filter(parameters == "v_CTI:C(block_type, Treatment('RG'))[T.TF]") %>%
  ggplot(aes(y = ddm_result, x = exp, fill = exp)) +
  ylab("Para. Est.") +
  stat_halfeye(position = position_dodge(width = 1),
               point_interval = "median_hdi",
...
( # non-decision time
p3<- psamples %>%
  filter(parameters == "t_CTI:C(block_type, Treatment('RG'))[T.TF]") %>%
  ggplot(aes(y = ddm_result, x = exp, fill = exp)) +
  ylab("Para. Est.") +
  stat_halfeye(position = position_dodge(width = 1),
               point_interval = "median_hdi",
...
(p1 | (p2 / p3)) + 
  plot_annotation(title = 'The DDM results of main and control experiments',
                  tag_levels = 'A')


##- exploratory: for CTI effects of DDM parameters of all the experiments ----
psamples <- big_ddm_postsamples %>%
  filter(effect == "CTI") %>%
  select(exp,
         "a_CTI:C(block_type, Treatment('RG'))[T.TF]",
...
# statistics of the posterior samples of Exp.1
dist_post_exp1 <- big_ddm_postsamples %>% 
  filter(exp == "v2") %>%
  filter(effect == "CTI") %>%
  select("a_CTI:C(block_type, Treatment('RG'))[T.TF]",
         "v_CTI:C(block_type, Treatment('RG'))[T.TF]",
  ...
  mutate(CI_lower = round(CI_low,3),
         CI_upper = round(CI_high,3)) %>%
  add_column(ddm_parameter = c("Threshold", "Drift rate", "Non-decision time")) %>%
  select(ddm_parameter, CI_lower, CI_upper) %>%
  pivot_wider(names_from = ddm_parameter,
              values_from = c(CI_lower, CI_upper))%>%
  select(c(1,4,2,5,3,6))

# statistics of the posterior samples of Control experiment
dist_post_ctl <- big_ddm_postsamples %>% 
  filter(exp == "control") %>%
  filter(effect == "CTI") %>%
  select("a_CTI:C(block_type, Treatment('RG'))[T.TF]",
         "v_CTI:C(block_type, Treatment('RG'))[T.TF]",
...
# statistics of the posterior samples of Experiment 3
dist_post_exp3 <- big_ddm_postsamples %>%
  filter(exp == "rule_tran") %>%
  filter(effect == "CTI") %>%
  select("a_CTI:C(block_type, Treatment('RG'))[T.TF]",
         "v_CTI:C(block_type, Treatment('RG'))[T.TF]",
...
# statistics of the posterior samples of Experiment 4
dist_post_exp4 <- big_ddm_postsamples %>%
  filter(exp == "stim_tran") %>%
  filter(effect == "CTI") %>%
  select("a_CTI:C(block_type, Treatment('RG'))[T.TF]",
         "v_CTI:C(block_type, Treatment('RG'))[T.TF]",
...
# Combining the tables from all the experiments to generate a nice table
stats.table = data.frame(matrix(nrow = 4, ncol = 7))
colnames(stats.table) = c("Experiment", "Threshold.lower", "Threshold.upper", "Drift Rate.lower", "Drift Rate.upper", "Non-decision time.lower", "Non-decision time.upper")
stats.table$Experiment = c("Exp1", "Ctl", "Exp3", "Exp4")
stats.table[1,2:7] = dist_post_exp1[1, ]
stats.table[2,2:7] = dist_post_ctl[1, ]
stats.table[3,2:7] = dist_post_exp3[1, ]
stats.table[4,2:7] = dist_post_exp4[1, ]
str(stats.table)

fun_round <- function(x) {formatC(x, format = "f", digits = 3)}

table_readypub <- nice_table(stats.table,
                             col.format.custom = 2:7,
                             format.custom = "fun_round",
                             separate.header = TRUE,
                             title = "Table 5\nThe result of drift diffusion modelling on the interaction between CTI and block type", 
                             footnote = "CT.lower = the lower bound of credible interval, CT.upper = the upper bound of credible interval.")

table_readypub
save_as_docx(table_readypub, path = paste0(here(), "/APA_table_DDM.docx"))
...
( # decision boundary
  p1<- psamples %>%
    ...
    ylab("CTI by block type interaction") +
    stat_halfeye(position = position_dodge(width = 0.5),
                 point_interval = "median_hdi",
                 .width = 0.89,
                 point_size = 4,
...
# compute the difference of decision boundary between stimulus transform and rule transform
...
psamples <- big_ddm_postsamples %>%
  ...
  select(exp,
         ...
         "a_incongruence_score_rule:C(block_type, Treatment('RG'))[T.TF]",
         "t_incongruence_score_rule:C(block_type, Treatment('RG'))[T.TF]") %>%
  mutate(v_incongruence_score_rule_TF = v_incongruence_score_rule + `v_incongruence_score_rule:C(block_type, Treatment('RG'))[T.TF]`,
         a_incongruence_score_rule_TF = a_incongruence_score_rule + `a_incongruence_score_rule:C(block_type, Treatment('RG'))[T.TF]`,
         t_incongruence_score_rule_TF = t_incongruence_score_rule + `t_incongruence_score_rule:C(block_type, Treatment('RG'))[T.TF]`) %>%
  pivot_longer(cols = contains("_"),
               names_to = "parameters",
               values_to = "ddm_result")
...
dist_post2 <- big_ddm_postsamples %>% # statistics of the posterior samples of stimulus transform experiment
  filter(exp == "stim_tran") %>%
  filter(effect == "ruleCon") %>%
  select("a_incongruence_score_rule",
         "v_incongruence_score_rule",
...
v_diffDist_RG <- psamples %>% # the difference between RG blocks of two experiments
...
a <- hdi(v_diffDist_RG$diff, ci = 0.95, verbose = TRUE)
b <- hdi(v_diffDist_RG$diff, ci = 0.89, verbose = TRUE)


v_diffDist_TF <- psamples %>% # the difference between TF blocks of two experiments
  filter(parameters == "v_incongruence_score_rule_TF") %>%
  group_by(exp) %>%
  mutate(row = row_number()) %>% # create a unique identifier for each data entry
  pivot_wider(names_from = exp, values_from = ddm_result) %>%
  select(-row) %>%
...
a_TF <- hdi(v_diffDist_TF$diff, ci = 0.95, verbose = TRUE)
b_TF <- hdi(v_diffDist_TF$diff, ci = 0.89, verbose = TRUE)
