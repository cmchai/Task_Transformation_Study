############### preprocessing script -- belief state stimulus transform ####################
...
ntrials <- 324 # number of trials for each subject

########################################################
#######  saving data for peer review ###################
########################################################
# load(file = paste0("D:/Ghent_Braem/Experiment/2nd_experiment/Experiment_stim_tran", "/result/bigdf.Rdata"))

data_exp4 <- bigdf %>%
  select(rt, accuracy, response, 
         block_type, block_id, trial_num, trial_nature, stim, rule,
         image_up_id, image_mid_id, image_low_id, image_target,
         CTI,
         answer, correct_key, subject) %>%
  mutate(exp = "exp4")
...
bigdf <- bigdf %>%
  mutate(# for the congruence effect on non-target images
...
# some descriptive of low acc participants
bigdf %>%
  filter(subject %in% lowacc_subject$subject) %>%
  group_by(subject, sona, start_block, block_type, trial_nature) %>%
  summarise(count = n(),
            nNA = sum(is.na(response)),
...
bigdf_clean_subject <- bigdf_ex_lowacc_sub %>% 
  ...
  mutate(CTI_bins = ntile(CTI, 6)) # put CTI into bins

save(bigdf_clean_subject, file = paste0(here(), "/result/bigdf_clean_subject.Rdata"))

(nrow(bigdf_ex_lowacc_sub) - nrow(bigdf_clean_subject))/ntrials # should equal to the number of subject level outliers

### demographic info after subject level data exclusion ###
load(file = paste0(here(), "/result/bigdf_clean_subject.Rdata"))
length(unique(bigdf_clean_subject$subject))

demo_age_clean <- bigdf_clean_subject %>%
  group_by(subject, gender, age) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  mutate(
...
demo_gender_clean <- demo_age_clean %>%
  group_by(gender) %>%
  summarise(count = n())
...
nrow(bigdf_clean_subject) - nrow(bigdf_ex_short_error_post) - 2631 - 24 # the number of excluded post-error trials
...
save(bigdf_clean, file = paste0(here(), "/result/bigdf_clean.Rdata"))
...
save(bigdf_clean_error, file = paste0(here(), "/result/bigdf_clean_error.Rdata"))
