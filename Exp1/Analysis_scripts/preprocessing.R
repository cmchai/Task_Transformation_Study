...
length(unique(bigdf_cleanSub$subject))

demo_age_clean <- bigdf_cleanSub %>%
  group_by(subject, gender, age) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  mutate(
...
nrow(bigdf_clean_subject) - nrow(bigdf_ex_short_error_post) - 2393 - 4 # the number of excluded post-error trials
